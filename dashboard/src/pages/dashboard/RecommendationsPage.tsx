      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem', flexWrap: 'wrap', gap: '0.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
          ...
          <button onClick={fetchRecommendations}
            ...
            style={{ ...btnBase, padding: '0.5rem 1rem', background: 'linear-gradient(135deg, #2563eb, #3b82f6)', color: 'white', fontWeight: 600, boxShadow: '0 2px 8px rgba(124,58,237,0.25)' }}>
      ...
      {recommendations.length > 0 && (
        <div data-tour="kpi-strip" style={{
          ...
          {[
            ...
            { label: 'Neutro', value: `${totalNeutral}`, color: '#6b7280', icon: null, tip: `Ações com score entre ${SCORE_SELL_THRESHOLD} e ${SCORE_BUY_THRESHOLD}.`, blur: false },
            ...
            { label: 'Top Score', value: `${topTicker?.ticker || '—'} ${fmt(topScore, 1)}`, color: '#3b82f6', icon: null, tip: 'Ação com maior score do dia.', blur: false },
      ...
      <div data-tour="search-bar" style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={signalFilter} onChange={e => setSignalFilter(e.target.value as any)}
          style={{
            ...
            background: theme.card || (darkMode ? '#1a1d27' : '#fff'), color: theme.text,
        ...
        <button onClick={() => handleSort(sortBy === 'score' ? 'return' : sortBy === 'return' ? 'ticker' : sortBy === 'ticker' ? 'vol' : 'score')}
          style={{ ...btnBase, padding: '0.55rem 0.75rem', background: theme.card || (darkMode ? '#1a1d27' : '#fff'), color: theme.text, border: `1px solid ${theme.border}` }}>
                ...
                >
                  ...
                  <td style={{ padding: '0.55rem 0.5rem', textAlign: 'right', fontWeight: 600, color: getSignalColor(getSignal(r.score)).text }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.35rem' }}>
                      {fmt(r.score, 2)}
                      <div style={{ width: 28, height: 4, borderRadius: 2, background: darkMode ? '#2a2e3a' : '#e2e8f0', overflow: 'hidden', flexShrink: 0 }}>
            ...
            <div key={r.ticker} style={{ ...cardStyle, marginBottom: '0.5rem', padding: '0.85rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.4rem', fontSize: '0.75rem' }}>
                ...
                <div>
                  ...
                  <ProValue isPro={hasAccess} style={{ fontWeight: 700, color: confidence >= 70 ? '#10b981' : '#6b7280' }} placeholder="••%">{confidence}%</ProValue>
                </div>
              </div>
