...
const AdminChatPage: React.FC = () => {
  ...
  const statusInfo = (s: string) => {
    ...
    if (s === 'closed') return { text: 'Encerrado', color: '#6b7280', bg: 'rgba(148,163,184,0.12)' };
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
          ...
          {activeTicket.status !== 'closed' && (
            <button onClick={() => handleClose(activeTicket.ticketId)}
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.35rem 0.7rem', borderRadius: 8, border: `1px solid rgba(148,163,184,0.3)`, background: 'transparent', color: '#6b7280', cursor: 'pointer', fontSize: '0.72rem', fontWeight: 600, WebkitAppearance: 'none' as any }}>
            ...
            return (
              <div key={m.id} style={{ display: 'flex', justifyContent: isAdmin ? 'flex-end' : 'flex-start' }}>
                <div style={{
                  ...
                  background: isAdmin ? 'linear-gradient(135deg, #10b981, #059669)' : (darkMode ? '#2a2e3a' : '#f1f5f9'),
                  color: isAdmin ? 'white' : theme.text,
                }}>
                  {!isAdmin && <div style={{ fontSize: '0.65rem', fontWeight: 600, color: '#3b82f6', marginBottom: '0.15rem' }}>{m.senderName || 'Cliente'}</div>}
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
            <textarea value={reply} onChange={e => setReply(e.target.value)} onKeyDown={handleKeyDown}
              ...
              style={{
                ...
                border: `1px solid ${theme.border}`, background: theme.card || (darkMode ? '#1a1d27' : '#fff'),
            ...
            <button onClick={handleReply} disabled={sending || !reply.trim()}
              style={{
                ...
                background: reply.trim() ? 'linear-gradient(135deg, #10b981, #059669)' : (darkMode ? '#2a2e3a' : '#e2e8f0'),
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem', flexWrap: 'wrap', gap: '0.5rem' }}>
        ...
        <button onClick={fetchTickets} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', padding: '0.45rem 0.75rem', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, background: 'linear-gradient(135deg, #2563eb, #3b82f6)', color: 'white', WebkitAppearance: 'none' as any }}>
        ...
        {([['all', 'Todos'], ['open', 'Abertos'], ['answered', 'Respondidos'], ['closed', 'Encerrados']] as const).map(([val, label]) => (
          <button key={val} onClick={() => setFilter(val)}
            style={{
              ...
              border: `1.5px solid ${filter === val ? '#3b82f6' : theme.border}`,
              background: filter === val ? 'rgba(59,130,246,0.1)' : 'transparent',
              color: filter === val ? '#3b82f6' : theme.textSecondary,
          ...
          {filtered.map(t => {
            ...
            return (
              <div key={t.ticketId} onClick={() => { setActiveTicket(t); setError(''); }}
                ...
                onMouseEnter={e => e.currentTarget.style.background = darkMode ? '#2a2e3a' : '#f1f5f9'}
