...
const ProGate: React.FC<ProGateProps> = ({ children, feature = 'Este recurso', darkMode = true, inline = false, storageKey }) => {
      ...
      <div style={{ position: 'relative', opacity: 0.5, pointerEvents: 'none', filter: 'blur(2px)' }}>
        ...
        <div style={{
          ...
          <span style={{
            ...
            borderRadius: 12, background: 'rgba(212,168,75,0.2)', color: '#d4a84b',
  ...
  return (
    <div style={{
      background: darkMode ? '#1a2626' : 'white',
      ...
      <button onClick={close} style={{
        ...
        color: darkMode ? '#5a7268' : '#8fa89c', cursor: 'pointer', padding: 4,
      }} aria-label="Fechar">
        <X size={18} />
      </button>
      <div style={{
        ...
        margin: '0 auto 1rem', boxShadow: '0 4px 16px rgba(212,168,75,0.3)',
      }}>
        <Crown size={28} color="white" />
      </div>
      <h2 style={{
        fontSize: 'clamp(1.1rem, 3vw, 1.3rem)', fontWeight: 700,
        color: darkMode ? '#e8f0ed' : '#121a1a', marginBottom: '0.5rem',
      }}>
        Recurso exclusivo Pro
      </h2>
      <p style={{
        fontSize: '0.85rem', color: darkMode ? '#8fa89c' : '#5a7268',
      ...
      <button onClick={() => window.location.hash = '#/dashboard/upgrade'} style={{
        ...
        fontSize: '0.9rem', fontWeight: 600, boxShadow: '0 4px 14px rgba(212,168,75,0.3)',
        transition: 'transform 0.15s, box-shadow 0.15s',
        WebkitAppearance: 'none', appearance: 'none',
      }}
        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(212,168,75,0.4)'; }}
        onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(212,168,75,0.3)'; }}
      >
        Fazer Upgrade <ArrowRight size={16} />
      </button>
      <p style={{ fontSize: '0.72rem', color: darkMode ? '#5a7268' : '#8fa89c', marginTop: '0.75rem' }}>
...
export const ProBadge: React.FC<{ small?: boolean }> = ({ small = false }) => (
  <span style={{
    ...
    borderRadius: 10, background: 'linear-gradient(135deg, #d4a84b, #b08a30)',
