...
AGENTS = {
    ...
    "COO": {
        ...
        "emoji": "⚙️",
        "color": "#8b5cf6",
        "focus": "Operações, pipelines DL, SLAs, automação, processos",
        "description": "Garante que pipelines de DL rodam sem falhas, monitora SLAs, automatiza processos e coordena operações.",
    },
    "CTO": {
...
PROJECT_CONTEXT = """
## Projeto: B3 Tactical Ranking — Dashboard DLOps para Bolsa Brasileira

### Visão Geral
Plataforma SaaS que usa Deep Learning (Transformer+BiLSTM) para analisar ~80 ações da B3 diariamente,
...
- DL: Transformer+BiLSTM treinado via Lambda/SageMaker (ml.m5.large), inferência em Lambda
...
- rank_sagemaker.py / rank.py — Geração do ranking DL
- train_sagemaker.py — Treinamento do modelo Transformer+BiLSTM
...
- SageMaker (treino DL): ~R$50/mês (ml.m5.large, ~1h/treino)
...
Você é um executivo de operações com expertise em DLOps, DevOps, SRE e automação de pipelines de dados.

SEUS FRAMEWORKS E EXPERTISE:
- DLOps: pipeline de DL end-to-end, model monitoring, retraining triggers
...
Você é um executivo técnico sênior com expertise em arquitetura serverless AWS, React, Python e sistemas de DL em produção.
...
- Backend: Python 3.11, boto3, pandas, numpy, scikit-learn, torch
...
- Pipelines DL, SLAs, monitoramento, DevOps → isso é do COO
...
══ SEU ESCOPO EXCLUSIVO ══
Você SOMENTE analisa e opina sobre:
- Pipeline de DL end-to-end: ingestão → curadoria → treino → ranking → validação
...
══ FRAMEWORKS ══
- DLOps, SRE (SLIs/SLOs/SLAs), Incident Management
...
- Stack técnico: React 18, TypeScript, Python 3.11, CDK, PyTorch
...
- Custos AWS, pricing, receita, finanças → isso é do CFO
- Marketing, aquisição, canais, growth → isso é do CMO
- Pipelines DL, SLAs, monitoramento operacional → isso é do COO
- Segurança, LGPD, vulnerabilidades, compliance → isso é do CISO
...
def _get_system_metrics() -> dict:
    ...
    metrics["infra"] = {
        ...
        "dl_model": "Transformer+BiLSTM via PyTorch",
...
def _build_metrics_context(metrics: dict) -> str:
  ...
  Modelo DL: {infra.get('dl_model', 'N/A')}
...
def _generate_agent_analysis(agent_id: str, metrics: dict, directive: str = "") -> dict:
    ...
    scope_map = {
        ...
        "COO": "operações, pipelines DL, SLAs, monitoramento, automação",
