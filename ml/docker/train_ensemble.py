...
def walk_forward_validation(X, y, hyperparameters, n_splits=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(X), 1):
        ...
        model = xgb.train(
            ...
            evals=[(dval, 'val')],
            early_stopping_rounds=20,  # Aumentado de 10 para 20
            verbose_eval=False
        )
...
def prepare_features_for_xgboost(X, y):
    """
    Features específicas para XGBoost (tree-based):
    ...
    if 'return_20d' in X_xgb.columns and 'vol_20d' in X_xgb.columns:
        X_xgb['sharpe_20d'] = X_xgb['return_20d'] / (X_xgb['vol_20d'] + 1e-6)
    
    if 'return_5d' in X_xgb.columns and 'return_20d' in X_xgb.columns:
        X_xgb['momentum_ratio'] = X_xgb['return_5d'] / (X_xgb['return_20d'] + 1e-6)
    
    if 'vol_5d' in X_xgb.columns and 'vol_20d' in X_xgb.columns:
        X_xgb['vol_ratio'] = X_xgb['vol_5d'] / (X_xgb['vol_20d'] + 1e-6)
    
    # Binning de volatilidade (tree-based gosta de categorias)
    if 'vol_20d' in X_xgb.columns:
        X_xgb['vol_bin'] = pd.qcut(X_xgb['vol_20d'], q=5, labels=False, duplicates='drop')
...
def prepare_features_for_lstm(X, y):
    """
    Features específicas para LSTM (sequencial):
    - Normalização Z-score
    - Features de tendência suavizadas
    - Sequências temporais
    """
    X_lstm = X.copy()
    
    # Normalização Z-score (LSTM precisa de features normalizadas)
    for col in X_lstm.columns:
        if X_lstm[col].dtype in ['float64', 'int64']:
            mean = X_lstm[col].mean()
            std = X_lstm[col].std()
            if std > 0:
                X_lstm[col] = (X_lstm[col] - mean) / std
    
    return X_lstm


def prepare_features_for_prophet(df_with_target):
    """
    Features específicas para Prophet (aditivo):
    - Componentes de tendência
    - Sazonalidade
    - Regressores externos
    """
    # Prophet usa formato específico (ds, y, regressors)
    prophet_df = pd.DataFrame({
        'ds': pd.date_range(start='2020-01-01', periods=len(df_with_target), freq='D'),
        'y': df_with_target['target'].values
    })
    
    # Adicionar regressores (features externas)
    if 'return_20d' in df_with_target.columns:
        prophet_df['momentum'] = df_with_target['return_20d'].values
    
    if 'vol_20d' in df_with_target.columns:
        prophet_df['volatility'] = df_with_target['vol_20d'].values
    
    return prophet_df


def train_xgboost(X_train, y_train, X_val, y_val, hyperparameters):
    """Treina modelo XGBoost com regularização forte"""
    logger.info("Treinando XGBoost com regularização...")
    
    # Preparar features específicas para XGBoost
    X_train_xgb = prepare_features_for_xgboost(X_train, y_train)
    X_val_xgb = prepare_features_for_xgboost(X_val, y_val)
    ...
    model = xgb.train(
        ...
        num_boost_round=hyperparameters.get('n_estimators', 100),
        evals=evals,
        early_stopping_rounds=20,  # Aumentado para evitar parada prematura
...
def train_lstm(X_train, y_train, X_val, y_val, hyperparameters):
    """Treina modelo LSTM com features normalizadas"""
    if not HAS_LSTM:
        logger.warning("LSTM não disponível, pulando...")
        return None, {'error': 'TensorFlow não disponível'}
    
    logger.info("Treinando LSTM com features normalizadas...")
    
    # Preparar features específicas para LSTM
    X_train_lstm = prepare_features_for_lstm(X_train, y_train)
    X_val_lstm = prepare_features_for_lstm(X_val, y_val)
    
    # Reshape para LSTM [samples, timesteps, features]
    X_train_reshaped = X_train_lstm.values.reshape((X_train_lstm.shape[0], 1, X_train_lstm.shape[1]))
    X_val_reshaped = X_val_lstm.values.reshape((X_val_lstm.shape[0], 1, X_val_lstm.shape[1]))
    
    # Criar modelo com mais camadas e dropout
    model = keras.Sequential([
        keras.layers.LSTM(128, activation='relu', return_sequences=True, input_shape=(1, X_train_lstm.shape[1])),
    ...
    # Early stopping e reduce learning rate
    ...
    # Métricas
    train_pred = model.predict(X_train_reshaped, verbose=0).flatten()
    val_pred = model.predict(X_val_reshaped, verbose=0).flatten()
    ...
    return model, {
        ...
        'feature_names': X_train_lstm.columns.tolist()
    }


def train_prophet(df_train, df_val, target_col='target'):
    """Treina modelo Prophet com regressores externos"""
    if not HAS_PROPHET:
        logger.warning("Prophet não disponível, pulando...")
        return None, {'error': 'Prophet não disponível'}
    
    logger.info("Treinando Prophet com regressores...")
    
    # Preparar dados para Prophet com regressores
    prophet_train = prepare_features_for_prophet(df_train)
    prophet_val = prepare_features_for_prophet(df_val)
    
    # Criar e treinar modelo com regressores
    model = Prophet(
        ...
        seasonality_mode='multiplicative'  # Melhor para séries financeiras
    )
    ...
    return model, {
        'train_rmse': float(train_rmse),
        'val_rmse': float(val_rmse),
        'regressors': ['momentum', 'volatility'] if 'momentum' in prophet_train.columns else []
    }
...
def main():
    ...
    # Ignorar argumentos desconhecidos do SageMaker
    args, unknown = parser.parse_known_args()
    if unknown:
        logger.info(f"Ignorando argumentos desconhecidos: {unknown}")
    ...
    # Salvar feature names do XGBoost para usar na predição
    xgb_feature_names = xgb_metrics.get('feature_names', X_train.columns.tolist())
    ...
    try:
        ...
        logger.error(f"Erro ao treinar Prophet: {e}")
        logger.warning("Continuando sem Prophet...")
    ...
    if 'xgboost' in models:
        # Preparar features específicas para XGBoost (mesmas usadas no treino)
        X_val_xgb = prepare_features_for_xgboost(X_val, y_val)
        # Garantir que as colunas estão na mesma ordem do treino
        X_val_xgb = X_val_xgb[xgb_feature_names]
        xgb_pred = models['xgboost'].predict(xgb.DMatrix(X_val_xgb))
        ensemble_val_pred += xgb_pred * ensemble_weights.get('xgboost', 0)
    
    if 'lstm' in models:
        # Preparar features específicas para LSTM
        X_val_lstm_prep = prepare_features_for_lstm(X_val, y_val)
        X_val_lstm = X_val_lstm_prep.values.reshape((X_val_lstm_prep.shape[0], 1, X_val_lstm_prep.shape[1]))
        lstm_pred = models['lstm'].predict(X_val_lstm, verbose=0).flatten()
        ensemble_val_pred += lstm_pred * ensemble_weights.get('lstm', 0)
    
    if 'prophet' in models:
        # Preparar features específicas para Prophet
        df_val_prophet_prep = X_val.copy()
        df_val_prophet_prep['target'] = y_val.values
        prophet_df = prepare_features_for_prophet(df_val_prophet_prep)
    ...
    # Salvar feature names específicos do XGBoost
    if 'xgboost' in models:
        with open(model_path / 'xgboost_features.json', 'w') as f:
